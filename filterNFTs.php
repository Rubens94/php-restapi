...
if ($_SERVER['REQUEST_METHOD'] == 'GET')
{
    ...
    $back = array();
    $next = array();
    $url = "http://localhost/apirest/filterNFTs.php?";
    $back['back'] = "$url";
    $next['next'] = "$url";

    $body = json_decode(file_get_contents("php://input"), true);
    $data = $body['attributes'] ?? null;

    if($data == null) {
        $query = "SELECT * FROM lazynfts a INNER JOIN collections b ON a.idcollection=b.ID";

        $urlStartAndEnd = "";
        if(isset($_GET['start']) && isset($_GET['end'])) {
            $query .= " WHERE a.TicketNumber BETWEEN $start AND $end";
        }

        $urlOrder = "";
        if($order == "") {
            $query .= " AND a.Minted = 0 ORDER BY a.TicketNumber ASC LIMIT $prdouctsPerPage OFFSET $ofset";
        } elseif(isset($_GET['order'])) {
            $query .= " AND a.Minted = 0 ORDER BY a.datePubublished, a.TicketNumber $order LIMIT $prdouctsPerPage OFFSET $ofset";
        ...
        $back['back'] .= $urlOrder;
        $next['next'] .= $urlOrder;
        $back['back'] .= $urlStartAndEnd;
        $next['next'] .= $urlStartAndEnd;
        ...
        if(isset($_GET['start']) && isset($_GET['end'])) {
            $sentence4 = $query;
            $search4 = "WHERE";
            $replace4 = "WHERE a.TicketNumber BETWEEN $start AND $end AND ";
            $query = str_replace($search4, $replace4, $sentence4);
            $urlStartAndEnd .= "&start=$order&end=$end";
        }
